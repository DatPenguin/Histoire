...
public class ImmigrationPool {
	...
	public void fillPool(){
		for(int i = 0 ; i < this.listePeuple.size() ; i++){
			if(listePeuple.get(i).getImigration() < 0){
				this.populationPool -= listePeuple.get(i).getImigration();
				listePeuple.get(i).setPopulation(listePeuple.get(i).getPopulation() + listePeuple.get(i).getImigration());	//la population serra bien retiré aux pays
			}
		}
	}
	...
	public void emptyPool(){
		...
		for(iteration = 0 ;iteration < this.listePeuple.size() ; iteration++){
			...
			if(this.listePeuple.get(index).getImigration() > 0){							// Si le peuple à besoin d'immigrés alors
				if(this.listePeuple.get(index).getImigration() > this.populationPool){		// Si la demande est supérieur à la quantité disponible
					...
					listePeuple.get(index).setPopulation(listePeuple.get(index).getPopulation() + + listePeuple.get(index).getImigration());		// On lui attribuera uniquement les immigrés qu'il demande si l'offre excède la demande
					this.populationPool -= listePeuple.get(index).getImigration();
				}
			}
		}
