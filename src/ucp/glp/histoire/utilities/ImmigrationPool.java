...
public class ImmigrationPool {
    ...
    /**
     * Cette méthode fera immigrer la population d'un pays si il est surchargé et remplira la populationPool en attente d'attribution
     */
    public void fillPool() {
        for (Peuple aListePeuple : this.listePeuple) {
            if (aListePeuple.getImmigration() < 0) {
                ...
                aListePeuple.setPopulation(aListePeuple.getPopulation() + aListePeuple.getImmigration());    //la population serra bien retirée aux pays
            }
        }
    }
    ...
    public void emptyPool() {
        ...
        for (int iteration = 0; iteration < this.listePeuple.size(); iteration++) {
